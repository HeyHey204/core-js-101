...
function findFirstSingleChar(str) {
  ...
  return firstSingleChar || null; // Return null if there are no non-repeated characters
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < reversedStr.length; i++) {
    let digit = parseInt(reversedStr[i], 10);
...
function getCommonDirectoryPath(pathes) {
  ...
  const shortestPath = directories
    .reduce((shortest, path) => (path.length < shortest.length ? path : shortest));

  let commonPath = '';
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < shortestPath.length; i++) {
    const currentDirectory = shortestPath[i];
    if (directories.every((path) => path[i] === currentDirectory)) {
