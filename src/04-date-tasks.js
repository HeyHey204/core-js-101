...
function angleBetweenClockHands(date) {
  ...
  const hourAngleDegrees = (hours * 30 + minutes * 0.5);
  const minuteAngleDegrees = (minutes * 6);

  let angleDiffDegrees = Math.abs(hourAngleDegrees - minuteAngleDegrees);
  if (angleDiffDegrees > 180) {
    angleDiffDegrees = 360 - angleDiffDegrees;
  }

  const angleDiffRadians = (angleDiffDegrees * Math.PI) / 180;

  return angleDiffRadians;
}
